...
fn build(
    ...
    let mut object: Object<'_> = class.new_instance(message)?.coerce_to_object()?;
...
pub(crate) fn to_js(env: &Env, error: &Error) -> napi::Error {
    ...
    build(env, "Error", family(error), error.code(), message.clone())
        .unwrap_or_else(|_| napi::Error::from_reason(message))
}
