...
class Health:
    ...
    @property
    def state(self) -> str: """`"ok"`, `"degraded"` or `"down"`."""
    @property
    def failures(self) -> int: """Consecutive unanswered verifications."""
    @property
...
class Device:
    ...
    @property
    def id(self) -> str: """The MAC it reports, uppercased."""
    @property
    def sku(self) -> str: """The SKU it is encoded under."""
    @property
    def name(self) -> str | None: """The name the configuration gives it."""
    @property
    def modes(self) -> list[str]: """The enabled modes, in preference order."""
    @property
...
class Served:
    ...
    @property
    def id(self) -> str: """The device it went to."""
    @property
    def mode(self) -> str: """The mode that served it."""
    @property
    def command(self) -> str: """The device file entry that was sent."""
    @property
...
class DeviceStatus:
    ...
    @property
    def id(self) -> str: """Which device answered."""
    @property
    def on(self) -> bool | None: """Whether it is on."""
    @property
    def brightness(self) -> int | None: """The level it reports, unmodified."""
    @property
    def color(self) -> Color | None: """The color, as three channels."""
    @property
    def color_temp_kelvin(self) -> int | None: """`0` means color mode."""
    @property
    def raw(self) -> dict[str, Any]: """The whole reply, every field included."""
    @property
...
class Config:
    ...
    def __init__(self) -> None: """Build the default configuration: `lan` alone."""
    @staticmethod
    def load() -> Config: """Read the file. A missing file is the default."""
    @staticmethod
    def load_from(path: str | os.PathLike[str]) -> Config:
        """Read the configuration from one path."""

    @property
    def default_modes(self) -> list[str]: """The modes a device without an entry gets."""
    @property
    def devices(self) -> list[str]: """The identity of every device the file names."""
    @property
...
class Catalog:
    ...
    @staticmethod
    def embedded() -> Catalog: """The catalog built into this build."""
    def skus(self) -> list[str]: """Every SKU that resolves, verified aliases included."""
    def has(self, sku: str) -> bool: """Whether the catalog holds this SKU."""
...
class SegmentStream:
    ...
    @property
    def zones(self) -> int: """How many zones the stream writes."""
    @property
    def rate_hz(self) -> float: """The rate the stream sends at."""
    @property
    def frames_sent(self) -> int: """How many frames reached the wire."""
    @property
    def frames_superseded(self) -> int: """How many frames a later frame replaced."""
    @property
    def error(self) -> str | None: """What stopped the stream, if anything did."""
    def set_all(self, colors: Sequence[Color]) -> None: """Write one color per zone."""
    def set_zone(self, index: int, color: Color) -> None: """Write one zone."""
    def fill(self, color: Color) -> None: """Write one color to every zone."""
...
class DeviceHandle:
    ...
    @property
    def id(self) -> str: """The identity the handle names."""
    @property
    def modes(self) -> list[str]: """The enabled modes, in preference order."""
    def health(self, mode: str) -> Health | None:
        """Its health in one mode. `None` when that mode never heard from it."""

    def serving_mode(self) -> str: """The mode a command would go over."""
    ...
    def spec(self) -> dict[str, Any]: """What the device file declares."""
    ...
    async def status(self) -> DeviceStatus: """Ask the device for its state."""
    def last_status(self) -> DeviceStatus | None: """The last status read."""
    async def power(self, on: bool) -> Served: """Turn the device on or off."""
    async def brightness(self, level: int) -> Served:
        """Set the brightness. Out of range is an error, never a clamp."""

    async def color(self, rgb: Color) -> Served: """Set one color on the device."""
...
class Govee:
    ...
    @staticmethod
    async def start(config: Config | None = None, catalog: Catalog | None = None) -> Govee:
        """Start the SDK.
    ...
    async def scan(self) -> list[Device]: """Scan every mode for devices."""
    async def scan_on(self, modes: Sequence[str]) -> list[Device]:
        """Run a discovery scan on the modes named."""

    def devices(self) -> list[Device]: """Every device known, across every mode."""
    def modes(self) -> list[str]: """The modes this build carries a transport for."""
    def problems(self) -> list[str]: """What is wrong with the configuration."""
    @property
    def config(self) -> Config: """The configuration in force."""
    @property
    def catalog(self) -> Catalog: """The device files this build carries."""
    def device(self, id: str) -> DeviceHandle: """A handle on one device."""
    def events(self) -> EventStream: """An async iterator over the SDK's events."""
