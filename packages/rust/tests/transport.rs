...
impl Rig {
    ...
    fn command(&self, name: &str, args: &Args) -> Encoded {
        ...
        govee_toolkit::codec::encode(device, Mode::Lan, name, args).expect("the command encodes")
    }
...
async fn the_cache_makes_a_restart_free_of_scanning() {
    let directory =
        std::env::temp_dir().join(format!("govee-toolkit-lan-restart-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&directory);
    let path = directory.join("devices.json");

    let rig = Rig::with_options(Policy::default(), |options| Options {
        cache: govee_toolkit::lan::Cache::load(&path).expect("an empty cache"),
        ..options
    })
    .await;
    rig.discover().await;
