...
//!
//! This crate carries the bytes [`govee_core`] produces. It discovers devices,
...
//! use govee_lan::{DeviceId, Transport, Verify};
//!
//! # async fn example() -> Result<(), Box<dyn std::error::Error>> {
//! let catalog = Catalog::embedded()?;
//! let transport = Transport::start(govee_lan::Options::default()).await?;
...
//! let on = govee_core::encode(device, Mode::Lan, "power", &Args::new().int("on", 1))?;
