...
/// The rig the patch tests load: two `segment` fixtures at 1 and 49, and one
/// `full` fixture at 97.
...
fn the_dimmer_at_zero_powers_the_device_off() {
    ...
    let look = look(
        &rig.fixtures()[2],
        &universe(97, &[0, 0, 255, 255, 255, 255]),
    );
    assert!(!look.on);
...
fn a_full_fixture_reads_its_six_channels() {
    ...
    let look = look(fixture, &universe(97, &[255, 0, 255, 10, 20, 30]));
    assert!(look.on);
...
fn the_white_channel_at_zero_carries_no_value() {
    let catalog = catalog();
    let rig = rig(&catalog);
    let look = look(&rig.fixtures()[2], &universe(97, &[255, 0, 0, 0, 0, 0]));
...
fn the_mode_channel_asks_for_a_resend_at_250() {
    ...
    let rig = rig(&catalog);
    let fixture = &rig.fixtures()[2];
    assert!(look(fixture, &universe(97, &[255, 250, 0, 0, 0, 0])).resend);
    assert!(!look(fixture, &universe(97, &[255, 9, 0, 0, 0, 0])).resend);
}
...
fn a_segment_fixture_reads_the_white_ahead_of_its_zones() {
    let catalog = catalog();
    let rig = rig(&catalog);
    let look = look(&rig.fixtures()[0], &universe(1, &[255, 0, 128, 10, 20, 30]));
    assert!(look.white_temp.is_some());
    assert_eq!(look.zones[0], [10, 20, 30]);
    let dark = look.black();
    assert_eq!(
        dark.white_temp, None,
        "a blackout does not turn the rig white"
    );
}
...
fn a_channel_the_packet_stops_short_of_reads_zero() {
    let catalog = catalog();
    let rig = rig(&catalog);
    let look = look(&rig.fixtures()[0], &[255, 0, 0, 10, 20]);
    assert!(look.on);
...
fn a_held_fixture_takes_no_look_after_the_signal_goes() {
    ...
    let look = look(
        &rig.fixtures()[2],
        &universe(97, &[255, 0, 255, 10, 20, 30]),
    );
...
fn a_blacked_segment_fixture_takes_every_zone_to_zero() {
    let catalog = catalog();
    let rig = rig(&catalog);
    let look = look(&rig.fixtures()[0], &universe(1, &[255, 0, 0, 10, 20, 30]));
    let quiet = look
        .quiet(SignalLoss::Black)
        .expect("`black` asks for a look");
