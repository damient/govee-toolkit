...
//!
//! What is specific to this mode: a device takes one connection at a time and
//! stops advertising while it is up, so the transport keeps one link per
//! device; writes are paced ([`pace`]); an advertisement carries the Bluetooth
//! address, not the Wi-Fi MAC this crate identifies a device by
//! ([`transport::Transport::bind`]); and a device that advertises the encoding
//! flag takes encoded frames only ([`encode`], [`session`]).
//!
...
/// The characteristic frames are written to, without a response.
pub const WRITE_CHARACTERISTIC: Uuid = Uuid::from_u128(0x0001_0203_0405_0607_0809_0a0b_0c0d_2b11);

/// The characteristic replies are notified on.
pub const NOTIFY_CHARACTERISTIC: Uuid = Uuid::from_u128(0x0001_0203_0405_0607_0809_0a0b_0c0d_2b10);

/// The length of a frame on this wire, in bytes, checksum included. This wire
/// has no MTU negotiation. [`HOST_COLOR_PROTYPE`] is the one exception.
pub const FRAME_LEN: usize = 20;

/// The first byte of a host colour frame, which is shorter than [`FRAME_LEN`]
/// and carries a sum where the others carry an XOR. `docs/protocol/ble.md` 8.
