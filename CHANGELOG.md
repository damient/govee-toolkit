...
| ------- | --------- | ------- |
| `govee-toolkit` (Rust) | [`packages/rust/CHANGELOG.md`](packages/rust/CHANGELOG.md) | 0.11.0 |
| `govee-toolkit-cli` (Rust) | [`packages/rust/crates/cli/CHANGELOG.md`](packages/rust/crates/cli/CHANGELOG.md) | 0.6.0 |
| `govee-toolkit-dmx` (Rust) | [`packages/rust/crates/dmx/CHANGELOG.md`](packages/rust/crates/dmx/CHANGELOG.md) | 0.1.0 |
| `govee-toolkit` (Python) | [`packages/python/CHANGELOG.md`](packages/python/CHANGELOG.md) | 0.3.0 |
| `govee-toolkit` (Node) | [`packages/node/CHANGELOG.md`](packages/node/CHANGELOG.md) | 0.2.0 |
...
- `H6022` declares `segments.groups: 15`, the width the `ble` mask and the
  `cloud` zone array reach. The lamp serves a 48-channel `segment` personality
  beside its 399-channel `pixel` one.
- `H61A0` declares `segments.groups: 15`, the width the `ble` mask reaches.
  ...
  48; repatch the fixture and rewrite the cue.
...
- `H6022` and `H61A0` record that a `colortemp` ends the armed segment channel,
  judged by eye. The H6022 shows dark on the arm that follows.
...
- `H6022`, the Table Lamp 2, over `lan`: power, brightness, colour, the white
  ...
  music command.
- `H6022` declares 132 zones over 132 addressable LEDs, a grid of 12 around one
  ring by 11 rings. The firmware drops a frame naming more zones than that.
