// One icon per capability key. They are inline: the site ships no icon font
// and no third-party sprite. A key the map does not hold gets no icon, so a
// capability a device file adds tomorrow still renders.
const SEGMENTS =
...
const PATHS = {
  ...
  // The two segment capabilities share one icon, and the color tells them
  // apart: they address the same zones.
  segments: SEGMENTS,
